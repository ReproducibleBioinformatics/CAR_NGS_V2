...
def main():
    ...
    usage_str = ' '.join([f'\033[93m<workdir>{RESET}', f'\033[93m<inputdir>{RESET}', f'\033[93m<outdir>{RESET}', f'\033[92m<adapter5>{RESET}', f'\033[92m<adapter3>{RESET}', f'\033[92m<seq_type>{RESET}', f'\033[38;5;208m<metadata>{RESET}', f'\033[92m<metadata_sep>{RESET}', f'\033[92m<threads>{RESET}', f'\033[92m<quiet>{RESET}'])

    if len(sys.argv) != 11:
        print(f'{WHITE}Usage: python skewer.py {usage_str}{RESET}\n')
        print(f'{YELLOW}This function executes the docker container skewer1 to remove sequencing adapters from RNAseq reads{RESET}\n')
        print(f'{WHITE}Arguments:{RESET}')
        print(f'\033[93mworkdir        {RESET} [io]  indicating the working folder')
        print(f'\033[93minputdir       {RESET} [in]  indicating where gzip fastq files are located')
        print(f'\033[93moutdir         {RESET} [out] indicating the folder where results will be written')
        print(f'\033[92madapter5       {RESET}       character string indicating the fwd adapter')
        print(f'\033[92madapter3       {RESET}       character string indicating the rev adapter')
        print(f'\033[92mseq_type       {RESET}       type of reads to be trimmed. Two options: \"se\" or \"pe\" respectively for single end and pair end sequencing.')
    ...
    args['adapter5'] = sys.argv[4]
    args['adapter3'] = sys.argv[5]
    args['seq_type'] = sys.argv[6]
    args['metadata'] = sys.argv[7]
    args['metadata_sep'] = sys.argv[8]
    args['threads'] = sys.argv[9]
    args['quiet'] = sys.argv[10]
