...
skewer <- function(workdir = NULL, inputdir = NULL, outdir = NULL, adapter5 = NULL, adapter3 = NULL, seq_type = NULL, metadata = NULL, metadata_sep = NULL, threads = NULL, quiet = NULL) {

  if (any(sapply(list(workdir, inputdir, outdir, adapter5, adapter3, seq_type, metadata, metadata_sep, threads, quiet), is.null))) {
    cat(WHITE, 'Usage: skewer(<workdir> <inputdir> <outdir> <adapter5> <adapter3> <seq_type> <metadata> <metadata_sep> <threads> <quiet>)', RESET, '\n\n', sep = '')
    cat(YELLOW, "This function executes the docker container skewer1 to remove sequencing adapters from RNAseq reads", RESET, "\n\n", sep = "")
    cat_col('Arguments:', color = WHITE)
    cat('\033[93mworkdir         [io]  indicating the working folder', RESET, '\n', sep = '')
    cat('\033[93minputdir        [in]  indicating where gzip fastq files are located', RESET, '\n', sep = '')
    cat('\033[93moutdir          [out] indicating the folder where results will be written', RESET, '\n', sep = '')
    cat('\033[92madapter5              character string indicating the fwd adapter', RESET, '\n', sep = '')
    cat('\033[92madapter3              character string indicating the rev adapter', RESET, '\n', sep = '')
    cat('\033[92mseq_type              type of reads to be trimmed. Two options: \\"se\\" or \\"pe\\" respectively for single end and pair end sequencing.', RESET, '\n', sep = '')
  ...
  docker_vals$adapter5 <- args$adapter5
  docker_vals$adapter3 <- args$adapter3
  ...
  cmd <- paste('docker run --rm', mount_str, 'ghcr.io/reproduciblebioinformatics/docker4seq-skewer-v2:latest bash /home/start.sh <inputdir> <outdir> <adapter5> <adapter3> <seq_type> <metadata> <metadata_sep> <threads> <quiet>')
