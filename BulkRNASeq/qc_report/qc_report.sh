...
USAGE_STR=$(echo -e "${YELLOW}<workdir>${RESET}" "${YELLOW}<inputdir>${RESET}" "${YELLOW}<outdir>${RESET}" "${ORANGE}<metadata>${RESET}" "${GREEN}<metadata_sep>${RESET}" "${GREEN}<threads>${RESET}" "${GREEN}<quiet>${RESET}")

if [ "$#" -ne 7 ]; then
    echo -e "${WHITE}Usage: bash qc_report.sh ${USAGE_STR}${RESET}\n"
    echo -e "${YELLOW}This function runs FastQC on each FASTQ file listed in the sample metadata file (matched via the SampleName column, optionally located under a SampleFolder subdirectory), then aggregates all results into a single MultiQC report. Output files are named <SampleFolder_>SampleName_fastqc.html/.zip.${RESET}\n"
    echo -e "${WHITE}Arguments:${RESET}"
    echo -e "${YELLOW}workdir        ${RESET} [io]  indicating the working folder"
    echo -e "${YELLOW}inputdir       ${RESET} [in]  indicating the folder where input data(fastq) are located"
...
cmd="docker run --rm ${mount_str} ghcr.io/reproduciblebioinformatics/docker4seq-qc_report-v2:latest bash /home/start.sh <inputdir> <outdir> <metadata> <metadata_sep> <threads> <quiet>"
