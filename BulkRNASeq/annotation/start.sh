...
# ------- Argument Checking ------- #
# All arguments are mandatory: exact count check via -ne 9.
...
if [ ! -d "$workdir" ]; then
    log_error "Working directory '$workdir' does not exist."
    exit 1
fi
...
if [ ! -f "$annotation_file" ]; then
    log_error "Annotation file '$annotation_file' does not exist."
    exit 1
fi
...
if [ -z "$gene_biotype" ]; then
    log_error "Missing required parameter: gene_biotype."
    exit 1
fi

# ------- Check for Sample Files in Input Directory ------- #
log_step "Verifying files in $input_dir..."
genes_count=$(find "$input_dir" -maxdepth 1 -type f -name "*.genes.results" | wc -l)
if [ "$genes_count" -eq 0 ]; then
    log_error "No *.genes.results files found in $input_dir."
    exit 1
fi
...
# exit 2 -> failure of extract_gene_mapping.R (1st external tool invoked below)
# exit 3 -> failure of generate_expression_tables.R (2nd external tool invoked below)
# ==============================================================================

# ------- Step 1: Extract Gene Mapping from Annotation File ------- #
gene_map_cache="${workdir}/gene_annotation_map.tsv"
log_step "Step 1: Running extract_gene_mapping (extract_gene_mapping.R)..."

Rscript /usr/local/bin/extract_gene_mapping.R \
    "$annotation_file" \
    "$gene_biotype" \
    "$gene_map_cache" \
    "$threads" \
...
if [ $? -ne 0 ] || [ ! -f "$gene_map_cache" ]; then
    log_error "Failed to build gene annotation mapping from '$annotation_file'."
    exit 2
fi
...
log_step "Step 2: Directly aggregating and annotating expression matrices..."

Rscript /usr/local/bin/generate_expression_tables.R \
    "$metadata" \
    "$metadata_sep" \
    "$input_dir" \
    "$results" \
...
if [ $? -ne 0 ]; then
    log_error "Failed to aggregate expression matrices."
    exit 3
fi
