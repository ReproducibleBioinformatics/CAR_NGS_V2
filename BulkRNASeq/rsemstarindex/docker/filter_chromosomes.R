...
if (length(args) < 3 || args[1] %in% c("-h", "--help")) { show_usage(); quit(status = 1, save = "no"); }
...
if (!file.exists(genome_fa)) {log_error(sprintf("FASTA file not found: '%s'", genome_fa)); quit(status = 1, save = "no");}
...
if (sum(keep) == 0) {log_error("Chromosome selection discarded all sequences. Terminating execution."); quit(status = 1, save = "no");}
