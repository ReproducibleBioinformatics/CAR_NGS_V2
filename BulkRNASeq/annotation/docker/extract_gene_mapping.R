...
url_decode <- function(str) {if (is.na(str)) return(NA_character_); utils::URLdecode(str)}
...
main <- function() {
  ...
  if (length(args) < 3 || length(args) > 5) { show_usage(); quit(status = 1)}
  ...
  if (tolower(quiet_arg) == "true") {assign("QUIET", TRUE, envir = .GlobalEnv)}
  if (!file.exists(annotation_file)) {log_error(paste("Input annotation file does not exist:", annotation_file)); quit(status = 1)}
  ...
  read_skip <- n_header
  dt <- tryCatch({
    data.table::fread(file = read_file, sep = "\t", header = FALSE, select = c(3, 9), col.names = c("feature", "attributes"), skip = read_skip, quote = "", fill = TRUE, showProgress = FALSE)
  }, error = function(e) {
  ...
  if (nrow(res_dt) == 0) {log_warn("Warning: The resulting mapping table is empty after filtering!")}
