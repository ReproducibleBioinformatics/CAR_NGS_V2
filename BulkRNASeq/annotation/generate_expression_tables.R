...
# ==============================================================================
# Aggregates and normalizes RSEM outputs driven by .genes.results and
# .isoforms.results files listed in sample metadata.
# ==============================================================================
...
# ------- Helper function to normalize separator ------- #
...
main <- function() {
  ...
  if (!quiet_clean %in% c("true", "false")) {
    assign("QUIET", "false", envir = .GlobalEnv)
    log_error(paste0("The quiet parameter must be 'true' or 'false' (provided: '", quiet_arg, "')"))
    quit(status = 1)
  }
  assign("QUIET", quiet_clean, envir = .GlobalEnv)

  # ------- Validate and Normalize Metadata Separator ------- #
  parsed_sep <- parse_separator_inplace(metadata_sep)
  if (is.null(parsed_sep)) {
    log_error(paste0("Invalid metadata separator provided: '", metadata_sep, "'. Allowed values: ',', ';', '\\t', 'tab'."))
    quit(status = 1)
  }
  ...
  cat(paste0("  ", CYAN, "Script Name     :", NC, " ", GREEN, SCRIPT_NAME, NC, "\n"))
  cat(paste0("  ", CYAN, "Metadata        :", NC, " ", YELLOW, metadata_file, NC, "\n"))
  cat(paste0("  ", CYAN, "Metadata Sep    :", NC, " '", YELLOW, metadata_sep, NC, "'\n"))
  cat(paste0("  ", CYAN, "Input Dir       :", NC, " ", YELLOW, input_dir, NC, "\n"))
  cat(paste0("  ", CYAN, "Results Dir     :", NC, " ", YELLOW, results, NC, "\n"))
  cat(paste0("  ", CYAN, "Biotype Filter  :", NC, " ", YELLOW, bio_type_arg, NC, "\n"))
  cat(paste0("  ", CYAN, "Mapping File    :", NC, " ", YELLOW, mapping_file, NC, "\n"))
  cat(paste0("  ", CYAN, "Threads         :", NC, " ", YELLOW, threads_arg, NC, "\n"))
  cat(paste0("  ", CYAN, "Quiet Mode      :", NC, " ", YELLOW, QUIET, NC, "\n"))
  log_sep("=", CYAN)
  log_info("Initializing Expression Table Generator")

  # ------- Validate Threads Parameter ------- #
  log_step("Validating parameters and environment...")
  threads_num <- suppressWarnings(as.integer(threads_arg))
  if (is.na(threads_num) || threads_num <= 0) {
    log_error(paste0("The threads parameter must be a positive integer (provided: '", threads_arg, "')"))
    quit(status = 1)
  }
  
  # Configure data.table multithreading allocation
  setDTthreads(threads_num)

  # ------- Check Results Directory ------- #
  if (!dir.exists(results)) {
    log_error(paste0("Results directory '", results, "' does not exist."))
    quit(status = 1)
  }

  # ------- Check Input Directory ------- #
  if (!dir.exists(input_dir)) {
    log_error(paste0("Input directory '", input_dir, "' does not exist."))
    quit(status = 1)
  }

  # ------- Check Metadata File ------- #
  if (!file.exists(metadata_file)) {
    log_error(paste0("Sample metadata file '", metadata_file, "' does not exist."))
    quit(status = 1)
  }

  # ------- Check Mapping File ------- #
  if (!file.exists(mapping_file)) {
    log_error(paste0("Mapping file '", mapping_file, "' does not exist."))
    quit(status = 1)
  }
  ...
  # exit 2 -> failure during file scanning, table compilation, or file writing operations
  # ==============================================================================
  ...
  if (nrow(metadata) < 2) {
    log_error("The experimental metadata contains fewer than 2 unique samples.")
    quit(status = 1)
  }
  ...
  log_step("Scanning RSEM result files to collect quantified gene and isoform IDs...")
  ...
  log_step("Extracting gene-level quantifications...")
  ...
  log_step("Extracting isoform-level quantifications...")
  ...
  for (i in seq_len(nrow(metadata))) {
    ...
    if (missing_isos > 0) {
      log_warn(paste0("ANOMALY: Sample '", col_label, "' is missing ", missing_isos, " isoforms. Imputing with 0."))
      total_iso_anomalies <- total_iso_anomalies + missing_isos
    }
  ...
  log_step("Writing compiled binary, flat tables, and updated metadata...")
